...
namespace XGSchool.Admin
{
    ...
    /// indexImgList 的摘要说明
    /// </summary>
    public class indexImgList : IHttpHandler
    {
            ...
            else
            {
                ...
                if (action == "AddNew")
                {
                    var data = new
                    {
                        Title = "添加图片信息",
                        Action = action,
                        Imgs = new { imgId = "1", imgDescription = "", colsBelong = "", pagesBelong = "" }
                    };
                ...
                else if (action == "Edit")
                {
                    ...
                    else
                    {
                        ...
                        string html = CommonHelper.RenderHtml("/admin/indeximgEdit.htm", data);
                        context.Response.Write(html);
                    }
                }
                ...
                else if (action == "Show")//列表展示
                {
                    ...
                    //}
                    //接收其余参数
                    string colsBelong = context.Request["colsBelong"];
                    string pagesBelong = context.Request["pagesBelong"];
                    string num = context.Request["num"];
                    //判断参数

                    string sql = "select * from imgTable where colsBelong=@colsBelong and pagesBelong=@pagesBelong ";
                    DataTable dt = SqlHelper.ExecuteDataTable(sql, new SqlParameter("@colsBelong", colsBelong), new SqlParameter("@pagesBelong", pagesBelong));
                    var data = new
                    {
                        Title = "图片展示",
                        Imgs = dt.Rows,
                        colsBelong=colsBelong,
                        pagesBelong=pagesBelong

                    };
                    string html = CommonHelper.RenderHtml("admin/indeximgList.htm", data);
                    context.Response.Write(html);
                }
                //参数错误
                else
